...
const TH = 'white-space:nowrap;padding:8px 10px;background:var(--panel-2);color:var(--text-dim);' +
           'font-size:11px;border-bottom:1px solid var(--border);text-align:right;position:sticky;top:0;z-index:1;';

// ── Line parsing ──────────────────────────────────────────────────────────────
// Rule (MANUAL.md §1): the raw CSV line carries an array of float values whose
// POSITION (not label) determines meaning:
//   1st float → SP   2nd float → KP   3rd float → Val   4th float → Avg   5th float → RMS
...
function renderTable() {
  ...
  tbody.innerHTML = S.records.map(r => `
    <tr data-id="${r.id}">
      ...
      <td style="text-align:right;padding:5px 10px;font-family:var(--mono);color:var(--text-dim);border-bottom:1px solid var(--border);">${r.n}</td>
      ...
      <td style="text-align:right;padding:5px 10px;font-family:var(--mono);font-weight:700;color:var(--cyan);border-bottom:1px solid var(--border);border-left:1px solid var(--border);">${fmt1(r.AVG)}</td>
...
function onGroupChange() { drawChart(); }
...
function commitMeasurement(f) {
  ...
  if (checked.length === 1) {
    ...
    Object.assign(rec, f);
    rec.checked = false;
    app.log(`${t('sp_overwrite_log')} N=${rec.n}  Avg=${fmt1(rec.AVG)} g`, 'ok');
  } else {
    rec = { id: ++S.recId, n: ++S.nCounter, checked: false, ...f };
    S.records.push(rec);
  }
  renderTable();
...
function exportCSV() {
  ...
  let csv = csvHeaderComment() + '#,Val,SP,Avg,KP,RMS\n';
  S.records.forEach(r => {
    csv += [r.n, fmt1(r.VAL), fmt1(r.SP), fmt1(r.AVG), fmt1(r.KP), fmt1(r.RMS)].join(',') + '\n';
  });
...
async function copyData() {
  ...
  const us = userSettings();
  let txt = `Test Speed: ${us.speed} in/min\tInitial Delay: ${us.delay} s\tAveraging Time: ${us.avg} s\n` +
            '#\tVal\tSP\tAvg\tKP\tRMS\n';
  S.records.forEach(r => {
    txt += [r.n, fmt1(r.VAL), fmt1(r.SP), fmt1(r.AVG), fmt1(r.KP), fmt1(r.RMS)].join('\t') + '\n';
  });
  try {
...
function drawChart() {
  ...
  names.forEach((gid, idx) => {
    ...
    ctx.strokeStyle = C.boxStr; ctx.fillStyle = C.boxFill; ctx.lineWidth = 1.4;
    ...
    ctx.strokeStyle = C.med; ctx.lineWidth = 2;
    ...
    ctx.fillText(typeof gid === 'string' ? gid : ('#' + gid), cx, H - pad + 15);
  ...
  buildSidebar(el) {
    ...
    el.innerHTML = `<div class="sidebar-top">
      ...
      <div class="panel">
        ...
        <label class="fl" style="margin-top:0;">${t('sp_speed_label')}</label>
        <div style="display:flex;align-items:center;gap:6px;">
          <input type="number" id="sp_speed" class="inp" value="90" step="1" min="0">
          <span style="font-size:11px;color:var(--text-mut);width:48px;">in/min</span>
        </div>
        <label class="fl">${t('sp_delay_label')}</label>
        <div style="display:flex;align-items:center;gap:6px;">
          <input type="number" id="sp_delay" class="inp" value="0.5" step="0.1" min="0">
          <span style="font-size:11px;color:var(--text-mut);width:48px;">s</span>
        </div>
        <label class="fl">${t('sp_avg_label')}</label>
        <div style="display:flex;align-items:center;gap:6px;">
          <input type="number" id="sp_avg" class="inp" value="5" step="0.5" min="0">
          <span style="font-size:11px;color:var(--text-mut);width:48px;">s</span>
        </div>
    ...
    el.innerHTML = `
      ...
      <div class="panel datalog-head-bar">
        ...
        <div class="datalog-actions">
          ...
          <button class="sbtn red" onclick="app.instr.deleteChecked()">${t('sp_del_checked')}</button>
