# -*- coding: utf-8 -*-
"""
build_and_publish_6_12.py
Build v1.0.0-rc.6.12 to build_tmp/release_out/ (with STAGE_ONLY=1),
upload to GitHub Release v1.0.0-rc.6.12,
and keep local dist/3M_Instrument_Logger.exe and version.json at v1.0.0-rc.6.11.
"""
import os
import sys
import json
...
TAG = "v1.0.0-rc.6.12"
VERSION_STR = "1.0.0-rc.6.12"
FILE_VERSION_STR = "1.0.6.12"
RELEASE_DATE = "2026-09-18"

BASE_VERSION_STR = "1.0.0-rc.6.11"
BASE_FILE_VERSION_STR = "1.0.6.11"

EXE_NAME = "3M_Instrument_Logger.exe"
STAGED_EXE = ROOT / "build_tmp" / "release_out" / EXE_NAME
LOCAL_EXE = ROOT / "dist" / EXE_NAME
BACKUP_KEEP_EXE = ROOT / "dist" / "3M_Instrument_Logger_v6.11_keep.exe"
...
BODY = (
    "## 3M Instrument Logger v1.0.0-rc.6.12\n\n"
    "### 업데이트 후 이전 버전으로 재시작되는 현상 완전 해결\n\n"
    "1. **PyInstaller `_MEI*` 잔여 폴더 소거**\n"
    "   - 파일 교체 전후로 `%TEMP%\\_MEI*` 임시 디렉터리를 완전 소거하여, 새 프로세스가 구버전 임시 폴더를 재사용하는 현상 차단\n\n"
    "2. **윈도우 탐색기 더블클릭과 동일한 완전 독립 셸 실행 (`UseShellExecute = $true`)**\n"
    "   - 프로세스 상속 없이 Windows Shell 컨텍스트에서 깨끗하게 기동하여 업데이트 직후 즉시 최신 버전으로 표시 보장\n\n"
    "3. **기존 안정화 내역 포함 (v6.7 ~ v6.11)**\n"
    "   - 실시간 다운로드 게이지 폴링 및 3초 카운트다운\n"
    "   - 팝업창 전면 고정 및 DLL 로드 오류 방지"
)
